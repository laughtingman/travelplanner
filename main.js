...
const app = new Vue({
	...
	methods: {
		...
		getPointDuration(point) {
			...
			let tzStart = this.getTimezoneByCity(point.cityStart), tzEnd = null;
		...
		getEndings(point) {
			...
			let endedPoints = this.points.filter(z=>{
				...
				let dateStartPoint = this.getStartDate(point), dateStartEnding = this.getStartDate(z), dateEndEnding = this.getEndDate(z);
	...
	computed: {
		costs() {
			let spent = 0, needPay = 0, transport = 0, living = 0, food= 0, other = 0;
		...
		pies() {
			...
			let relativeSize = ((this.costs.transport / this.costs.total) * 314.159),
			startingPoint = 0;
			...
			relativeSize = ((this.costs.living / this.costs.total) * 314.159),
			...
			startingPoint+= relativeSize;
			relativeSize = ((this.costs.food / this.costs.total) * 314.159),
			...
			startingPoint+= relativeSize;
			relativeSize = ((this.costs.other / this.costs.total) * 314.159),
...
function loadCurrencies() {
	...
	getJSON("https://www.cbr-xml-daily.ru/daily_json.js").then(data => {
